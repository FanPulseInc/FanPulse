...
const SPORT_ICONS = [
  { icon: ICONS.FOOTBALL, route: "/football" },
  { icon: ICONS.BASCETBALL, route: "/basketball" },
  { icon: ICONS.RUGBY, route: "/american-football" },
  { icon: ICONS.MOTO, route: "/motorsport" },
  { icon: ICONS.TENIS, route: "/tennis" },
] as const;
...
const ESPORT_ICON_TO_CATEGORY = ["CS2", "Dota 2"] as const;
...
export default function Page() {
  ...
  const showMaintenance = () => {
    setMaintenance(t("maintenance_message"));
  };
    ...
    <main className="min-h-screen w-full overflow-hidden bg-white">
      ...
      <div className="w-full px-4 py-6 md:px-8 lg:px-12">
        <div className="relative space-y-6">
          {/* bg glow */}
          <div className="pointer-events-none absolute -top-20 left-1/2 h-72 w-72 -translate-x-1/2 rounded-full bg-brand-red/10 blur-3xl" />
          ...
          <div className="relative p-4 md:p-6">
            <div className="grid items-stretch gap-8 md:grid-cols-3">
                    ...
                    <Tab
                      onClick={() => {
                        if (favCategoryNames.length > 0) {
                          router.push("/forum?filter=favourite");
                        } else {
                          showMaintenance();
                        }
                      }}
              ...
              {/* ESPORT */}
              <HomeCard
                onClick={() => router.push("/esport/cs2")}
                header={
                  <img
                    src="icons/esport_icon.png"
                    className="h-[20px] w-[140px] object-contain transition-all duration-500 group-hover:scale-105 group-hover:-translate-y-0.5"
                    alt="Esport"
                  />
                }
                tabs={
              ...
              <HomeCard
                ...
                header={
                  <img
                    src="icons/forum_icon.png"
                    className="h-[20px] w-[100px] object-contain transition-all duration-500 group-hover:scale-105 group-hover:-translate-y-0.5"
                    alt="Forum"
                  />
                }
                tabs={
            ...
            <div className="flex w-max gap-2 marquee">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="flex shrink-0 gap-2">
                  <Banner src="/banners/beton.png" />
                  <Banner src="/banners/vbet.png" />
                  <Banner src="/banners/sportshop.png" />
                  <Banner src="/banners/fit.png" />
                  <Banner src="/banners/roullete.png" />
                </div>
              ))}
            </div>
          </div>
...
hover:-translate-y-2
hover:shadow-[0_25px_60px_rgba(175,41,42,0.18)]
...
"
    >
      ...
      <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100">
        <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-brand-red/10 blur-2xl" />
        <div className="absolute -bottom-12 -left-12 h-40 w-40 rounded-full bg-brand-red/10 blur-2xl" />
      </div>

      {/* header */}
      <div className="relative z-10 flex min-h-[50px] items-center justify-center gap-2 text-brand-red">
        {header}
      </div>

      {/* tabs */}
      <div className="relative z-10 mb-3 mt-3 flex items-center justify-center text-base font-medium">
        <div className="flex gap-5 text-brand-red">
          {tabs}
        </div>
      </div>
      ...
      {/* icons */}
      <div
        className={`relative z-10 mb-4 flex min-h-[40px] flex-col items-center justify-center gap-3 ${isForum ? "" : "flex-row"
          }`}
      >
        {icons}
      </div>
      ...
      {/* image */}
      <div className="relative mt-auto overflow-hidden rounded-[20px]">
        <img
          src={image}
          alt=""
          className="
