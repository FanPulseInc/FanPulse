...
const ESPORT_ICONS = [
  { icon: <img key="cs2" src="icons/cs2.jpg" className="w-6 h-6 object-contain" />, route: "/esport/cs2" },
  { icon: <img key="dota2" src="icons/dota2.png" className="w-6 h-6 object-contain" />, route: "/esport/dota" },
] as const;

/* category name → forum filter value (must match Category.name from API) */
...
export default function Page() {
  ...
  /* resolve favourite category names */
        ...
        <div className="p-4 md:p-6">
          <div className="grid md:grid-cols-3 gap-8 items-stretch">
            ...
            <div
              ...
              className="bg-[#efefef] rounded-[24px] border-2 border-brand-red p-4 h-full flex flex-col cursor-pointer hover:shadow-lg transition-shadow"
            >
              ...
              <div className="flex items-center justify-center text-base font-medium mb-3">
                <div className="flex gap-5 text-brand-red">
                  <span
                    className="cursor-pointer hover:underline"
                    onClick={(e) => { e.stopPropagation(); showMaintenance(); }}
                  >
                    {t("tab_all")}
                  </span>
                  <span
              ...
              <div className="flex gap-3 mb-4 justify-center min-h-[40px]">
                {SPORT_ICONS.map((item, i) => (
                  <div
                    ...
                    className="w-10 h-10 shrink-0 flex items-center justify-center border-2 border-brand-red rounded-full text-lg cursor-pointer hover:bg-brand-red/10 transition-colors"
                  >
                    {item.icon}
                  </div>
                ))}
            ...
            >
              ...
              <div className="flex gap-4 mb-4 justify-center min-h-[40px]">
                {ESPORT_ICONS.map((item, i) => (
                  <div
                    ...
                    className="w-10 h-10 shrink-0 flex items-center justify-center border-2 border-brand-red rounded-xl text-lg cursor-pointer hover:bg-brand-red/10 transition-colors"
                  >
                    {item.icon}
                  </div>
                ))}
            ...
            >
              ...
              {/* sport icons → forum filtered by that sport */}
              <div className="flex gap-3 mb-4 justify-center min-h-[40px]">
                {SPORT_ICONS.map((item, i) => (
                  <div
                    key={`forum-sport-${i}`}
                    onClick={(e) => {
                      e.stopPropagation();
                      router.push(`/forum?category=${encodeURIComponent(SPORT_ICON_TO_CATEGORY[i])}`);
                    }}
                    className="w-10 h-10 shrink-0 flex items-center justify-center border-2 border-brand-red rounded-full text-lg cursor-pointer hover:bg-brand-red/10 transition-colors"
                  >
                    {item.icon}
              ...
              {/* esport icons → forum filtered by that esport */}
              <div className="flex justify-center gap-3 mb-4 min-h-[40px]">
                {ESPORT_ICONS.map((item, i) => (
                  <div
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      router.push(`/forum?category=${encodeURIComponent(ESPORT_ICON_TO_CATEGORY[i])}`);
                    }}
                    className="w-10 h-10 shrink-0 flex items-center justify-center border-2 border-brand-red rounded-xl text-lg cursor-pointer hover:bg-brand-red/10 transition-colors"
                  >
                    {item.icon}
